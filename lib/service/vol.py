...
def robust_vol_calc(daily_returns: pd.Series,
    ...
    vol = _apply_min_vol(vol, vol_abs_min = vol_abs_min)
    
    if vol_floor:
        # Clip vol to a minimum value (that is 5% quantile of vol)
        vol = _apply_vol_floor(
            vol,
...
def _apply_min_vol(vol: pd.Series,
...
def _backfill_vol(vol: pd.Series) -> pd.Series:
...
def mixed_vol_calc(
    ...
    slow_vol_days = slow_vol_years * 256.0 # 256 = Business days in a year
